...
- (void)setupAppearanceStyles:(NSDictionary<NSString *, BRUIStyle *> *)styles {
	[styles enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, BRUIStyle * _Nonnull style, BOOL * _Nonnull stop) {
		if ( [key isEqualToString:BRStyleKeyDefault] || [key hasPrefix:BRStyleKeyControlsPrefix] || [key hasPrefix:BRStyleKeyBarControlsPrefix] ) {
			return;
		}
		...
		if ( !appearanceProxyClassName ) {
			NSLog(@"UIAppearance proxy class hierarhcy not found for style key %@", key);
			return;
		}
		...
		if ( containerClassNames.count > 0 ) {
			...
			for ( NSString *containerClassName in containerClassNames ) {
				...
				if ( !containerClass ) {
					NSLog(@"UIAppearanceContainer class %@ not available (style key %@)", containerClassName, key);
					return;
				}
		...
		if ( [(Class)appearanceProxyClass instancesRespondToSelector:@selector(setUiStyle:forState:)] ) {
			...
			NSLog(@"Class %@ does not seem to conform to BRUIStylish or BRUIStylishControl (style key %@)", appearanceProxyClassName, key);
		}
	}];
}
